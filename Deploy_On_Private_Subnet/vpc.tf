...
resource "aws_subnet" "public" {
  ...
  vpc_id                  = "${aws_vpc.demo.id}"
  tags {
    Name = "${var.name-prefix}-subnet-pub-${count.index}"
  }
}
...
resource "aws_subnet" "private" {
  ...
  vpc_id                  = "${aws_vpc.demo.id}"
  tags {
    Name = "${var.name-prefix}-subnet-priv-${count.index}"
  }
}
...
resource "aws_route_table" "public" {
  vpc_id = "${aws_vpc.demo.id}"
  tags {
    Name = "${var.name-prefix}-public_route_table_main"
  }
}
...
resource "aws_route_table" "private" {
  ...
  count ="${length(var.azs)}" 
  tags { 
    Name = "${var.name-prefix}-private_subnet_route_table_${count.index}"
  }
}
